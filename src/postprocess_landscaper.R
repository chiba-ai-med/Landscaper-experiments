...
cov_file     <- args[4]   # cov.tsv
allstates_file <- args[5] # Allstates.tsv
e_file       <- args[6]   # E.tsv
basin_file   <- args[7]   # Basin.tsv
mg_file      <- args[8]   # major_group.tsv
coord_file   <- args[9]   # Coordinate.tsv
igraph_file  <- args[10]  # igraph.RData
eb_file      <- args[11]  # EnergyBarrier.tsv
asmg_file    <- args[12]  # Allstates_major_group.tsv
...
cell_time <- as.numeric(read.table(cov_file, header = FALSE)[, 1])
...
seurat.integrated$cell_time   <- cell_time
...
# Optional: h, J, g for covariate models
landscaper_dir <- dirname(allstates_file)
h_path <- file.path(landscaper_dir, "h.tsv")
J_path <- file.path(landscaper_dir, "J.tsv")
g_path <- file.path(landscaper_dir, "g.txt")

if (file.exists(h_path)) {
    landscaper_data$h <- as.numeric(read.table(h_path, header = FALSE)[, 1])
}
if (file.exists(J_path)) {
    landscaper_data$J <- as.matrix(read.table(J_path, header = FALSE))
    mode(landscaper_data$J) <- "numeric"
}
if (file.exists(g_path)) {
    landscaper_data$g_cov <- as.numeric(read.table(g_path, header = FALSE)[, 1])
}
