...
# Loading
load(infile1)
bindata_DAPT <- read.table(infile2, header=FALSE)
...
# Setting
dir.create("plot/DAPT_cov/bindata/")

# Plot DAPT
for(i in seq_len(ncol(bindata_DAPT))){
     filename1 <- paste0("plot/DAPT_cov/bindata/", i, ".png")
     filename2 <- paste0("plot/DAPT_cov/bindata/", i, "_splitby.png")
     ...
     g <- DimPlot(seurat.integrated, reduction = "umap", group.by=groupname, split.by="sample",
         ncol=4, label=TRUE, pt.size=2, label.size=6, cols=c(4,2)) + NoLegend()
